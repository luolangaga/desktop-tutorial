...
Console.WriteLine(Figgle.FiggleFonts.Standard.Render("ASG Web!"));
...
builder.Services.AddSwaggerGen(c =>
{
    c.SwaggerDoc("v1", new Microsoft.OpenApi.Models.OpenApiInfo
    {
        Title = "ASG ???????-???API???",
        Version = "V 1.9.7",
        Description = "?????????????ASP.NET.Core??????ASG??????????????????????????????????? sqlserver????????identity?????????????",
    });
    var file = Path.Combine(AppContext.BaseDirectory, "ASGºó¶Ë.xml");
