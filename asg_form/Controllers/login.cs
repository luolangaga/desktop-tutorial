...
namespace asg_form.Controllers
{
    public class login : ControllerBase
    {
        ...
        public async Task<ActionResult<string>> put_password_email2([FromBody]string email)
        {
...
");
   ...
   [Route("api/v1/password/ok")]
        [HttpPost]
        public async Task<ActionResult<string>> put_password1([FromBody]password_email password)
        {
        ...
        [Route("api/v2/password/ok")]
        [HttpPost]
        public async Task<ActionResult<string>> put_password2([FromBody]password_email_new password)
        {
            var user = await userManager.FindByNameAsync(password.user_name);
           var r= await userManager.ResetPasswordAsync(user, password.Token, password.New_Password);
            if(r.Succeeded)
            {
                return "成功";
       ...
       public class password_email
        {
         ...
         public class password_email_new
        {
...
");
                ...
                else
                {
                    ...
                    if (userManager.Users.Any(a => string.Equals(a.Email,newuser.EMail) && a.EmailConfirmed == true))
                    {
...
");
        ...
        public async Task<ActionResult<post_user>> getuser()
        {
            ...
            return new post_user { id = id.ToInt64(),Base64=user.UserBase64, name = user.UserName, chinaname = user.chinaname, email = user.Email, isadmin = isadmin ,Roles=roles, officium = user.officium ,Integral=user.Integral};
...
");
        ...
        var opuser= userManager.Users.Where(x => x.officium == opname).ToList();
            ...
            foreach (var auser in opuser)
            {
                ...
                user.Add(new post_user { id = auser.Id, chinaname = auser.chinaname, name = auser.UserName, isadmin = isadmin, email = auser.Email, Roles = (List<string>)roles , Integral = auser.Integral });
